...
class AvgView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for location in locations:
            ...
            print("Cantidad de Reg: ", locationData.count())

            avgVal = locationData.aggregate(Avg("avg_value"))["avg_value__avg"]
            print("avgVal: ", avgVal)

            greaterCount = Data.objects.filter(
                station__in=stations, measurement__name=selectedMeasure.name,  time__gte=start_ts, time__lte=end_ts, avg_value__gte=avgVal)
            print("Cantidad de mayor al promedio: ", greaterCount.count())
            data.append(
                {
                    "name": f"{location.city.name}, {location.state.name}, {location.country.name}",
                    "lat": location.lat,
                    "lng": location.lng,
                    "population": stations.count(),
                    'CantidadReg': locationData.count(),
                    'avg': round(avgVal if avgVal != None else 0, 2),
                    'CantMayorAVG': greaterCount.count() if greaterCount.count() != None else 0,
                }
            )
